...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Aggiungere questa funzione per garantire la retrocompatibilità con i vecchi client
function ensureLegacyCompatibility(data) {
  // Per rilevazioni di velocità
  if (data.type === "speed") {
    // Assicurati che i campi siano compatibili con il vecchio formato
    data.zone = data.zone || "Non specificata";
  }
  
  // Per cambi gomme
  if (data.type === "tyre") {
    // Supporta sia 'box' che 'zone' per compatibilità
    if (!data.box && data.zone) {
      data.box = data.zone;
    }
  }
